...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase().toList.groupBy(char => char).toList.map(pair => (pair._1, pair._2.length)).sortBy(_._1)
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
  // concat the list of string to a big string
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    dictionary.map(w => (w, wordOccurrences(w))).groupBy(pair => pair._2).mapValues(pair => pair.map(_._1))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    for ((char, freq) <- occurrences) {
      for (i <- 1 to freq) {
        for (w <- combinations(occurrences.filter(x => x._1 > char))) {
          result = (List((char, i)) ++ w) :: result
        }
      }
    }
    result
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = (for {
    (char, x_freq) <- x
    y_freq = (y.toMap[Char, Int]).getOrElse(char, 0)
    if (x_freq > y_freq)
  } yield (char, x_freq - y_freq)) sorted
